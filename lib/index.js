...
function processCLIParamsAndExecute () {
  if (argv.length !== 4) {
    console.log('node run start -- [path-and-filename-of-dependencies-file] [search-directory]')
    console.log('\nThe dependencies file should contain one or more lines - with each line consisting of a dependency name, followed by an \'@\', followed by a version.  So for example: glob@1.0.0')
    exit(1)
  }
  ...
  indexerService.search(searchDirectory, packageJsonFiles, targetDependencies)
}
