...
function processCLIParamsAndExecute () {
  if (argv.length !== 4) {
    ...
    console.log('\n  node run start -- [packages-file] [search-directory]')
    console.log('\nwhere...')
    console.log('  - [packages-file] is the path and filename of the file containing the packages you want to search for (required). This file should consist of one or more lines, each consisting of a package name, followed by an \'@\', followed by the version you\'re looking for.')
    console.log('  - [search-directory] is the path to a directory containing the projects you wish to search (required).')
    exit(1)
  }

  const dependenciesFile = argv[2]
  console.log(`packages file: ${dependenciesFile}`)
  ...
  for (let i = 0; i < targetDependencies.length; i++) {
    ...
    if (index === -1) {
      ...
      console.log(`  - ignoring malformed package: ${d}`)
    } else {
  ...
  console.log(`  - ${targetDependencies.length} target package${targetDependencies.length === 1 ? '' : 's'} found`)
  if (targetDependencies.length < 1) {
    console.log('  - aborting run as a minimum of 1 target package is required')
    exit(1)
  }
  ...
  console.log(`  - ${packageJsonFiles.length} package.json file${packageJsonFiles.length === 1 ? '' : 's'} found`)
  if (packageJsonFiles.length < 1) {
    console.log('  - aborting run as no package.json files were found')
    exit(1)
  }
  ...
  console.log(`  - number of package.json files found with a target package, but with a different version: ${results.packageMatchCount}`)
  console.log(`  - number of package.json files found with a target package of the specified version: ${results.packageAndVersionMatchCount}`)
}
